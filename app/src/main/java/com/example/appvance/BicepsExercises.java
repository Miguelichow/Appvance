...
/**
 * Classe que maneja los ejercicios de bíceps
 * @author Miguel Prieto Horcajo
 */
public class BicepsExercises extends AppCompatActivity implements View.OnClickListener {

    NumberPicker npBiceps, npSets, npReps;
    ImageButton ibCheck;
    String nombre, sets, reps;
    SQLiteHelperExercises sqliteHelperEx;
    String[] data = {" ", "Dominadas supinas", "Dominadas supinas en L", "Dominadas pronas",
            "Dominadas supinas agarre cerrado", "Específico de bíceps"};
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setTitle("Ejercicios de bíceps");
        setContentView(R.layout.activity_biceps_exercises);

        etNew = findViewById(R.id.etNew);

        npBiceps = findViewById(R.id.npBiceps);
        npBiceps.setMinValue(0);
        npBiceps.setMaxValue(data.length - 1);
        npBiceps.setWrapSelectorWheel(true);
        npBiceps.setDisplayedValues(data);
    ...
    /**
     * Controla la función del click sobre los distitnos componentes
     * @param v Vista
     */
    @Override
    public void onClick(View v) {
        if(etNew.getText().toString().isEmpty()) {
            nombre = data[npBiceps.getValue()];
        ...
        } else if(!etNew.getText().toString().isEmpty() && data[npBiceps.getValue()].equals(" ")){
